"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var common_1 = require('@angular/common');
var forms_1 = require('@angular/forms');
var components_module_1 = require('./../components/components.module');
var parameters_title_component_1 = require('./parameters-title.component');
var parameters_view_component_1 = require('./parameters-view.component');
// import { ParametersListComponent } from './parameters-list.component';
var parameters_container_component_1 = require('./parameters-container.component');
// import { ParameterNewComponent } from './parameter-new.component';
// import { ParameterEditComponent } from './parameter-edit.component';
var parameters_routing_1 = require('./parameters.routing');
var parameter_service_1 = require('./parameter.service');
var number_separator_1 = require('../components/number.separator');
//import { CarouselModule } from 'ng2-bootstrap/ng2-bootstrap';
var ParameterModule = (function () {
    function ParameterModule() {
    }
    ParameterModule = __decorate([
        core_1.NgModule({
            imports: [
                common_1.CommonModule,
                forms_1.FormsModule,
                parameters_routing_1.ParametersRouting,
                components_module_1.ComponentModule
            ],
            declarations: [
                // CampaignNewComponent,
                // CampaignEditComponent,
                parameters_title_component_1.ParametersTitleComponent,
                parameters_container_component_1.ParametersContainerComponent,
                parameters_view_component_1.ParametersViewComponent,
                number_separator_1.CommaSeparatedNumberPipe
            ],
            providers: [
                parameter_service_1.ParameterService
            ]
        }), 
        __metadata('design:paramtypes', [])
    ], ParameterModule);
    return ParameterModule;
}());
exports.ParameterModule = ParameterModule;
//# sourceMappingURL=parameters.module.js.map